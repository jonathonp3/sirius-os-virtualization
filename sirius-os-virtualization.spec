...
# Install README
install -Dpm 0644 %{SOURCE11} %{buildroot}%{_docdir}/%{name}/README.md
...
%changelog
* Mon Sep 21 2026 Jonathon P <jonathon@sirius-os> - 2.0.0-2
- License changed from GPLv3 to GPL-3.0-only (valid SPDX identifier).
- Ship README.md as %doc.
- Removed leftover %post and %postun. On rpm-ostree, these scriptlets run in a
  build chroot, not on the live system, so systemctl daemon-reload has
  no effect. The provisioning service handles daemon-reload at first
  boot, and the dormant uninstaller handles it after removal.
