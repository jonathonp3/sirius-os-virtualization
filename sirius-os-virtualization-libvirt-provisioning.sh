#!/bin/bash
# Sirius-OS: libvirt provisioning with dynamic subnet
set -euo pipefail

echo "🔧 Sirius-OS libvirt provisioning starting..."

# --- 1. Install firewalld services ---
echo "📦 Installing firewalld services..."
mkdir -p /etc/firewalld/services
cp /usr/share/sirius-os/container-http.xml /etc/firewalld/services/
cp /usr/share/sirius-os/container-https.xml /etc/firewalld/services/
cp /usr/share/sirius-os/container-https-alt.xml /etc/firewalld/services/
cp /usr/share/sirius-os/quick-share.xml /etc/firewalld/services/

# --- 2. Install firewalld zone ---
echo "📦 Installing firewalld zone..."
mkdir -p /etc/firewalld/zones
cp /lib/firewalld/zones/libvirt.xml /etc/firewalld/zones/
...
sed "s/100/${SUBNET}/g" /usr/share/sirius-os/default-net.xml > /etc/libvirt/qemu/networks/default.xml
